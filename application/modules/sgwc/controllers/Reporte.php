...
class reporte extends MX_Controller
{
	...
	{		
		if($this->session->userdata('id')):
			...
			$data["repfot_mp"] = $this->menu->crea_menu_repfot($data['idperfil'],$data['iduser']);
			$data['css'] = '';
			$data['js'] = '';
	...
	{		
		if($this->session->userdata('id')):
			...
			$data['css'] = '';
			$data['js']='';	
			$data["repfot_sub"] = $this->menu->crea_submenu_repfot($data['idperfil'],34);
			$this->template->load('template','reporte_fot',$data);			
		else:
			redirect('login/index','refresh');
		endif;
	}
	...
	{		
		if($this->session->userdata('id')):
			...
			$this->template->load('template','estandares',$data);			
		else:
			redirect('login/index','refresh');
		endif;
	}
	...
	public function carga_estandares()
	{
		...
		if($proyecto==1 or $proyecto==4):
			...
			$this->load->view('carga_estandares_a',$data);
		else:
			$data['fuentes']=$this->reporte_model->desplega_fuentes($proyecto);
			$data['clases']=$this->reporte_model->desplega_clases($proyecto);
			$data['segmentos']=$this->reporte_model->desplega_segmentos($proyecto);			
			$this->load->view('carga_estandares_s',$data);
		endif;
	}
	
	public function carga_segmentos(){
		$proyecto=$this->input->get('proyecto');										
		$car='';
		foreach($this->input->get('carretera') as $carretera):
			$car .= $carretera.",";
		endforeach;
		$car=trim($car,',');
		$segmentos=$this->reporte_model->desplega_segmentos_carretera($proyecto,$car);	
		$datasource = array();
