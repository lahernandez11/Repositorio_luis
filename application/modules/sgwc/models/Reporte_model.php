...
class Reporte_model extends CI_Model
{
	...
	public function desplega_proyecto_permiso($iduser){
		$query = $this->db->query("SELECT * FROM vw_foto_base WHERE idusuario=".$iduser." AND idbase NOT IN (3)");
		//idestado=1
		return $query->result();
	}
	
	public function desplega_proyecto_siap($iduser){
		$query = $this->db->query("SELECT * FROM vw_foto_base WHERE idusuario=".$iduser." AND idbase IN (1)");
		//idestado=1
		return $query->result();
	}
	...
	public function desplega_clases($base){
		$query = $this->db->query("SELECT * FROM foto_clase WHERE idestado=1 AND idbase=".$base." ORDER BY clase");
		return $query->result();
	}
	...
	public function desplega_carreteras($proyecto){
		$query = $this->db->query("SELECT * FROM foto_carretera WHERE idbase=".$proyecto." ORDER BY carretera");
		return $query->result();
	}
	
	public function desplega_segmentos_notificacion($proyecto){
		$query = $this->db->query("SELECT * FROM [opi].[dbo].[foto_segmento] WHERE idbase=".$proyecto." ORDER BY tramo");
		return $query->result();
	}
	...
	public function desplega_segmentos_carretera($proyecto,$carretera){
		$query = $this->db->query("SELECT * FROM foto_segmento WHERE idbase=".$proyecto." AND carreteraID IN (".$carretera.") ORDER BY tramo");
	...
	public function desplega_id(){
		$query=$this->db->query("SELECT codeID FROM sgwc_inspecion_visual");
		return $query->result_array();
	}
}
/*
