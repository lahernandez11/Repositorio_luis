$(document).ready(function(){
	...
	$("select#proyecto").change(function(){
		...
		$("div#informacion").load(base_url+'sgwc/eventos/carga_estandares?proyecto='+proyecto);		
	});
	...
	$("div#informacion").on('click','#enviar',function(){		
		...
		{errores=errores+1;}
			...
			if(errores>0){
				...
				//alert(datos);
				//$("#enviar").attr('href','http://172.20.73.117/reporte_fotografico/dev_gmmm_repfot_eventos1.php?'+datos)
				$("#enviar").attr('href','http://ns5002111.ip-192-99-17.net/sgwc_repfot/dev_gmmm_repfot_eventosp6.php?'+datos)
...
//////////////////////////////////////////
	...
	$("#d_proyecto").change(function(){		
		...
		$.post(base_url+'sgwc/eventos/numero_fotos',datos,function(data){
	...
	$("div#ordena").on('change','#fotos',function(){		
		datos="proyecto="+$(this).val()+"&fotos="+$("select#fotos option:selected" ).text();
		$.post(base_url+'sgwc/eventos/carga_ordenacion',datos,function(data){
			$('div#orden').html(data);
		});
	});
	...
	$("div#orden").on('click','#mod-orden',function(){
		...
		if(estado==1){		
			...
			if(errores>0){
				...
				$.getJSON(base_url+"sgwc/eventos/modifica_orden",datos,function(data){
					if(data.msg=='ok'){
